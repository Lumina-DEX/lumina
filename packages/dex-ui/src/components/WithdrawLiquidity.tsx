	...
	useEffect(() => {
		const subscription = Dex.subscribe((snapshot) => {
			...
			if (result) {
				...
				const from = result.amountAOut / 10 ** 9
				const to = result.amountBOut / 10 ** 9
		...
		<>
			<div className="flex flex-row justify-center w-full ">
				<div className="flex flex-col p-5 gap-5 items-center">
					...
					<div>
						<span>Liquidity withdrawed : {toFixedIfNecessary(liquidityMinted, 2)}</span>
					</div>
					<ButtonStatus onClick={calculateLiquidity} text={"Calculate Amount"}></ButtonStatus>
					<ButtonStatus onClick={addLiquidity} text={"Withdraw Liquidity"}></ButtonStatus>
				</div>
			</div>
		</>
	)
