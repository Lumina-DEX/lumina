 ...
 */
export class UserOrder extends SmartContract {
  @state(Field)
  ...
  async addOrder(
    ...
    const tokenId = TokenId.derive(sender)
    this.tokenId.assertEquals(tokenId, "Wrong contract for deposit")
  ...
  async executeOrder(orderA: AddOrder, orderB: AddOrder, witnessA: MerkleMapWitness, witnessB: MerkleMapWitness) {
    const amountIn = orderA.amountBuy
    const balance = getBalance(orderA.sender, TokenId.derive(orderA.tokenBuy))
    await this.send({ to: orderB.sender, amount: orderB.amountBuy })
    const tokenBId = TokenId.derive(orderA.tokenSell)
    const orderTokenB = new UserOrder(this.address, tokenBId)
    await orderTokenB.updateOrder(orderA, orderB, witnessA, witnessB)
  }
