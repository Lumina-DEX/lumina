...
export async function generateRewardRoot(start: UInt64, end: UInt64, farmAddress: PublicKey) {
  ...
  let totalByBlock = UInt64.zero
  let blockHeight = UInt32.zero
  // create total account
  const totalAccount = "0x0"
  userList.set(totalAccount, [])
  for (let index = 0; index < events.length; index++) {
    ...
    if (time.equals(blockHeight).not().toBoolean()) {
      // we store old total
      ...
      total.push(totalInfo)
      // if time differs it's a new block
      blockHeight = time
      totalByBlock = UInt64.zero
    }
    const farmInfo = {
      time: time.toBigint(),
