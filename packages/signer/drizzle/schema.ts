...
export const dbNetworks = pgTable("Network", {
	network: text("network", { enum: networkEnums }).primaryKey(),
	createdAt: timestamp("created_at").notNull().default(sql`CURRENT_TIMESTAMP`)
})
...
export const pool = pgTable(
	"Pool",
	{
		...
		status: text("status", {
			enum: ["pending", "confirmed", "deployed", "unconfirmed"] as const
		}).notNull(),
		network: text("network", { enum: networkEnums })
			.notNull()
			.references(() => dbNetworks.network)
...
export const poolKey = pgTable(
	...
	(t) => [check("PoolOrFactory", sql`(num_nonnulls(${t.poolId}, ${t.factoryId}) = 1);`)]
)
