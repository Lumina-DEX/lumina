...
export const deployFactoryAndTransaction = async ({
	...
	const existingFactory = await db.drizzle.query.factory.findFirst({
		where: eq(factory.network, network)
	})
	...
	if (existingFactory) {
		logger.log("Factory already exists in database", existingFactory.publicKey)
		factoryPublicKey = PublicKey.fromBase58(existingFactory.publicKey)
		factoryPublicKeyBase58 = existingFactory.publicKey
		factoryId = existingFactory.id

		// Retrieve factory private key from poolKey table
		const factoryKeys = await db.drizzle.query.poolKey.findFirst({
			where: eq(poolKey.factoryId, factoryId),
			with: {
				signer1: true,
				signer2: true
			}
		...
		if (!factoryKeys) {
			throw new Error("Factory private key not found in database")
		}

		// Decrypt the factory private key (double decryption)
		const encryptedFields = encryptedKeyToField(factoryKeys.encryptedKey)
		const decryptB = Encryption.decrypt(
			{ publicKey: PublicKey.fromBase58(factoryKeys.generatedPublic2).toGroup(), cipherText: encryptedFields },
			PrivateKey.fromBase58(process.env.SIGNER_2_PRIVATE_KEY!) // TODO: Get from secure vault
		)
		const decryptA = Encryption.decrypt(
			{ publicKey: PublicKey.fromBase58(factoryKeys.generatedPublic1).toGroup(), cipherText: decryptB },
			PrivateKey.fromBase58(process.env.SIGNER_1_PRIVATE_KEY!) // TODO: Get from secure vault
		)
		const decryptedKey = Encoding.stringFromFields(decryptA)

		factoryPrivateKey = PrivateKey.fromBase58(decryptedKey)
		logger.log("Retrieved and decrypted factory private key from database")
	} else {
	...
	const signatures = await db.drizzle
		...
		.where(
			and(
				eq(multisig.network, network),
				eq(multisig.deadline, sql`(SELECT MAX(deadline) FROM ${multisig} WHERE network = ${network})`)
			)
		)
	...
	const updateSignerData = UpdateSignerData.fromJSON(JSON.parse(signatures[0].data))
	const messageHash = updateSignerData.hash()
	...
	const minaTransaction = await db.drizzle.transaction(async (txOrm) => {
		...
		// Insert factory if it doesn't exist
		if (!existingFactory) {
		...
		const minaTx = await Mina.transaction(
			...
			async () => {
				...
				await zkFactory.deploy({
					...
					protocol: PublicKey.fromBase58(deployer), // TODO: Set proper protocol address from config
					delegator: PublicKey.fromBase58(deployer), // TODO: Set proper delegator address from config
					approvedSigner: root,
