...
### How CI rebuilds differ from local rebuilds

The generated hardware config (`hosts/generated/*-hardware.nix`) is gitignored, so CI does not have
it at checkout time. `rebuild-host.sh` handles this automatically: when the file is missing it SSHs
into the target and runs `nixos-generate-config --show-hardware-config` to fetch it before building.

Because `nixos-generate-config --show-hardware-config` only produces filesystem and kernel-module
information (not boot-loader settings), the base hardening module sets
`boot.loader.grub.enable = lib.mkDefault false`. CI deploys therefore activate the new generation
and restart services without touching the boot loader — the host keeps booting from whatever
`nixos-anywhere` installed during the initial rollout. When the full hardware config IS present
(operator workstation, after bootstrap), it can override this with the correct grub device.

`nixos-rebuild --target-host` uses the `ssh://` transport, which runs `nix-store --serve --write`
on the remote. This rejects unsigned locally-built store paths unless `require-sigs = false` is set
in the remote's nix.conf. The base hardening module sets both `nix.settings.require-sigs = false`
and `nix.settings.trusted-users = [ "root" "@wheel" ]`. On a host where these settings do not yet
exist (first deploy), `rebuild-host.sh` handles the chicken-and-egg: it SSHs in, replaces the
immutable `/etc/nix/nix.conf` store symlink with a mutable copy that includes the settings, and
restarts `nix-daemon`. After `nixos-rebuild switch` succeeds, the system activation restores the
proper symlink (which now includes the settings from the NixOS module).

The NixOS modules use `builtins.getEnv` to read `LUMINA_SIGNER_IMAGE_REF`,
`LUMINA_SIGNER_ADMIN_AUTHORIZED_KEY`, and other values at evaluation time. Nix flake evaluation is
pure by default, so `rebuild-host.sh` passes `--impure` to `nixos-rebuild` to make these env vars
visible.

### Required GitHub environment secrets

Each deploy workflow references a GitHub environment (`signer-zeko-testnet`, `signer-mina-mainnet`,
`signer-zeko-mainnet`). The following secrets must be configured per environment:

| Secret            | Description                                                |
| ----------------- | ---------------------------------------------------------- |
| `SSH_HOST`        | IP or hostname of the target server                        |
| `SSH_PORT`        | SSH port                                                   |
| `SSH_USER`        | SSH user (typically `lumina-admin`)                        |
| `SSH_PRIVATE_KEY` | Ed25519 private key for SSH access                         |
| `SSH_KNOWN_HOSTS` | Known-hosts entry for the target                           |
| `TARGET_HOSTNAME` | Public hostname (e.g. `zeko-testnet.signer.luminadex.com`) |

Optional (recommended):

| Secret                               | Description                                                                         |
| ------------------------------------ | ----------------------------------------------------------------------------------- |
| `LUMINA_SIGNER_ADMIN_AUTHORIZED_KEY` | SSH public key for the admin user. If absent, CI derives it from `SSH_PRIVATE_KEY`. |
| `LUMINA_SIGNER_CI_AUTHORIZED_KEY`    | SSH public key for a dedicated CI user, if separate from admin.                     |
