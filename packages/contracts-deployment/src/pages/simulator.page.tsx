...
const Simulator = () => {
	...
	return (
		<>
			<div
				className="flex flex-col min-w-[360px] w-screen max-w-[600px]  h-[600px] p-5  rounded"
			...
			>
				<div className="m-5">
					...
					<input
						type="number"
						onChange={(ev) => {
							setData({ ...data, liquidityMina: ev.target.valueAsNumber })
						}}
						value={data.liquidityMina}
					></input>
				</div>
				<div className="m-5">
					...
					<input
						type="number"
						value={data.liquidityToken}
						onChange={(ev) => {
							setData({ ...data, liquidityToken: ev.target.valueAsNumber })
						}}
					></input>
				</div>
				<div className="m-5">
					...
					<input
						...
						value={data.amountIn}
						onChange={(ev) => {
							setData({ ...data, amountIn: ev.target.valueAsNumber })
						}}
					></input>
				...
				<div className="m-5">
					...
					<span>{data.amountOut.toFixed(2)}</span>
				</div>
			</div>
		</>
	)
