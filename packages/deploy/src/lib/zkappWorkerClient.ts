...
export default class ZkappWorkerClient {
	...
	fetchAccount({ publicKeyBase58 }: { publicKeyBase58: string }): ReturnType<typeof fetchAccount> {
	...
	fetchAccountToken({ publicKeyBase58 }: { publicKeyBase58: string }): ReturnType<typeof fetchAccount> {
	...
	async setNewDelegator(
		...
		return await this._call("setNewDelegator", {
			user,
			oldUser,
			newUser,
			deadlineSlot,
