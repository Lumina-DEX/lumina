...
export class PoolTokenHolder extends SmartContract implements IPool {
	 ...
	 */
	@method async swapFromTokenToToken(
		frontend: PublicKey,
		taxFeeFrontend: UInt64,
	...
	@method async withdrawLiquidity(
		...
		const sender = this.sender.getUnconstrained()
		const amountToken = this.withdraw(
			sender,
			liquidityAmount,
			amountTokenMin,
			reserveTokenMin,
		...
		const pool = new Pool(this.address)
		const amountMina = await pool.withdrawLiquidity(
			sender,
			liquidityAmount,
			amountMinaMin,
			reserveMinaMin,
	 ...
	 * @param supplyMax maximum liquidity in the pool
	 */
	@method async withdrawLiquidityToken(
		liquidityAmount: UInt64,
	 ...
	 */
	@method.returns(UInt64) async subWithdrawLiquidity(
		sender: PublicKey,
		liquidityAmount: UInt64,
		amountToken1Min: UInt64,
		reserveToken1Min: UInt64,
		...
		this.emitEvent(
			...
			new SubWithdrawLiquidityEvent({
				sender,
				amountTokenOut: amountToken,
				amountLiquidityIn: liquidityAmount
			})
		)
