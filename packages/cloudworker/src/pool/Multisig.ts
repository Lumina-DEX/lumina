					...
					.concat(
						this.updateSigner
							...
							.concat(
								this.updateProtocol
									.toFields()
									.concat(this.updateDelegator.toFields().concat(this.updateFactory.toFields()))
							)
					)
			)
	}
	...
	toFields(): Field[] {
		return this.contractAddress
			...
			.concat(
				this.tokenId
					.toFields()
					.concat(this.newVkHash.toFields().concat(this.deadlineSlot.toFields()))
			)
	}
...
export class UpdateAccountInfo extends Struct({
	...
	toFields(): Field[] {
		return this.oldUser
			.toFields()
			.concat(this.newUser.toFields().concat(this.deadlineSlot.toFields()))
	}
...
export class UpdateSignerData extends Struct({
	...
	toFields(): Field[] {
		return this.oldRoot
			.toFields()
			.concat(this.newRoot.toFields().concat(this.deadlineSlot.toFields()))
	}
